...
dateTimeMessage = str(calendar.day_name[today.weekday(
)]) + ", " + str(today.strftime("%B")) + " " + str(today.day)
...
querystring = {"id": "2172797", "units": "%22metric%22 or %22imperial%22",
               "mode": "xml%2C html", "q": "Ashbourne,ie", "cnt": "15"}
...
class MyMainApp(App):
    ...
    def get_weather_data(self, *args):
        try:
            ...
            temp1 = str(round(data["list"][0]["main"]
                              ["feels_like"] - 273.15, 2)) + " C"
            temp2 = str(round(data["list"][2]["main"]
                              ["feels_like"] - 273.15, 2)) + " C"
            temp3 = str(round(data["list"][4]["main"]
                              ["feels_like"] - 273.15, 2)) + " C"
            ...
            if nextTimeReading == "00:00:00":
                ...
                self.tomorrow_info = str(round(
                    data["list"][0]["main"]["temp"] - 273.15, 2)) + " C" + "\n" + data["list"][0]["weather"][0]["description"]
                self.tomorrow_image = "http://openweathermap.org/img/wn/%s@2x.png" % (
                ...
                for x in data["list"]:
                    ...
                    if todayDate != x["dt_txt"][0:10] and str(x["dt_txt"][11:19]) == "12:00:00":
                        ...
                        self.tomorrow_info = str(round(
                            x["main"]["temp"] - 273.15, 2)) + " C" + "\n" + x["weather"][0]["description"]
                        self.tomorrow_image = "http://openweathermap.org/img/wn/%s@2x.png" % (
                            x["weather"][0]["icon"])
                        print("2moro " + self.tomorrow_info +
                              " " + self.tomorrow_image)
                        break
    ...
    def get_covid_data(self, *args):
        try:
            response = requests.request(
                "GET", urlCovid, headers=covidHeaders, params=covidQuerystring)
            data = response.json()
            ...
            self.weather_error = "There was an issue connecting to the COVID API."
            ...
            for event in events:
                ...
                if len(start) < 18:
                    ...
                    start_str = datetime.strptime(
                        start[0:19], "%Y-%m-%dT%H:%M:%S")
                self.upcomingEvents = self.upcomingEvents + \
                    str(start_str.strftime("%b %d %H:%M")) + \
                    " • " + event['summary'] + "\n"
        except:
